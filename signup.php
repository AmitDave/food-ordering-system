<?php 
    ...
    if(isset($_POST)&& !empty($_POST)){
        ...
        $number=stripslashes(mysqli_real_escape_string($connection,$_POST['number']));
        $address=stripslashes(mysqli_real_escape_string($connection,$_POST['address']));
        ...
        if($pass==$conpass){
            $pass=crypt($pass, '$6$rounds=5000$randomsalt$');    
            $sql="INSERT INTO `user`(`id`,`name`,`email`,`number`,`address`, `password` ,`type`)VALUES(3,\"$name\", \"$email\", \"$number\" , \"$address\" ,\"$pass\", 2)";
    ...
    <body>
        <div class="container">
          <form class="form-signin" role="form" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
            <h2 class="form-signin-heading">Welcome To FoodEx</h2>
            <h3 class="form-signin-heading">Signup for the best food in town</h3>
